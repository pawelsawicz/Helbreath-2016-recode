...
CMapData::CMapData(class cMain * pGame)
{
	int i;
	m_pGame = pGame;
	...
	{	m_stFrame[i][OBJECTSTOP].m_sMaxFrame		= 14;
		...
		m_stFrame[i][OBJECTDAMAGEMOVE].m_sFrameTime	= 50;
		m_stFrame[i][OBJECTRUN].m_sMaxFrame			= 7;
		m_stFrame[i][OBJECTRUN].m_sFrameTime		= 42;
		m_stFrame[i][OBJECTATTACK].m_sMaxFrame		= 7;
		m_stFrame[i][OBJECTATTACK].m_sFrameTime		= 78;
		...
		m_stFrame[i][OBJECTDAMAGE].m_sFrameTime		= 70;
	...
	{	sDist = (abs(sCenterX - dX)+abs(sCenterY - dY))/2;
		...
		{	m_pData[dX][dY].m_iEffectFrame++;
			...
			{	m_pData[dX][dY].m_cDynamicObjectFrame++;
				...
				case DYNAMICOBJECT_ARESDENFLAG1:
					if (m_pData[dX][dY].m_cDynamicObjectFrame >= 4)
					{	m_pData[dX][dY].m_cDynamicObjectFrame  = 0;
					}
					break;

				case DYNAMICOBJECT_ELVINEFLAG1:
					if (m_pData[dX][dY].m_cDynamicObjectFrame >= 8)
		...
		{	switch (m_pData[dX][dY].m_cOwnerAction) {
			...
			{	if ((dwTime - m_pData[dX][dY].m_dwOwnerTime) >= (dwFrameTime +dwFrameTime))
				...
				{	if (   (m_sRectX <= dX) && ((m_sRectX + 20) >= dX)
					...
					{	if (m_pData[dX][dY].m_cOwnerAction == OBJECTDYING) //10
						{	m_pData[dX][dY].m_wDeadObjectID        = m_pData[dX][dY].m_wObjectID;
							m_pData[dX][dY].m_sDeadOwnerType       = m_pData[dX][dY].m_sOwnerType;
							m_pData[dX][dY].m_cDeadDir             = m_pData[dX][dY].m_cDir;
							m_pData[dX][dY].m_sDeadAppr1           = m_pData[dX][dY].m_sAppr1;
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTSTOP ) { // OBJECTSTOP = 1 // 00497334
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTMOVE ) { //2 //004977BF
					switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTRUN )  // 2   //00497E34
				{	switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTATTACKMOVE )  //8 //004980A5
				{	switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:
				...
				if ( (m_pData[dX][dY].m_cOwnerAction == OBJECTATTACK) ) { //3 00498685
					switch (m_pData[dX][dY].m_sOwnerType) {
					case 65: // IceGolem
						if (m_pData[dX][dY].m_cOwnerFrame == 2)
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTDAMAGE )  // 6  00499159
				{	switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:  // Men
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTDAMAGEMOVE ) { // 7 004997BD
					switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTMAGIC )  // 4 00499D51
				{	switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:
				...
				if ( m_pData[dX][dY].m_cOwnerAction == OBJECTDYING )  // 10 // 00499F5D
				{	switch (m_pData[dX][dY].m_sOwnerType) {
					case 1:
					case 2:
					case 3:
